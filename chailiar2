// ============================================================
// CHAI-LIAR v5.0 - ADVANCED DSA EDITION
// Features: TreeMap sales, Max-Heap leaderboard, Trie search,
// Undo/Redo stacks, hashCode/equals, persistent menu.
// ============================================================
...
public class Project1 extends JFrame {
    ...
    public Project1() {
        setTitle("☕ CHAI-LIAR v5.0 | Advanced DSA Edition");
        setSize(1400, 900);
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> new Project1().setVisible(true));
    }
}
...
// ============================================================
// USER CLASS (with equals/hashCode)
// ============================================================
...
// ============================================================
// MENU ITEM (with equals/hashCode)
// ============================================================
...
// ============================================================
// SALES REPORT MANAGER (using TreeMap for time-based queries)
// ============================================================
    ...
    public void loadReports() {
        ...
        try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(salesFile))) {
            ...
            if (obj instanceof ArrayList) {
                // Legacy format: convert to TreeMap
                List<SaleTransaction> oldSales = (ArrayList<SaleTransaction>) obj;
                salesByTime = new TreeMap<>();
                for (SaleTransaction sale : oldSales) {
                    LocalDateTime time = sale.getTimestamp().toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime();
                    salesByTime.computeIfAbsent(time, k -> new ArrayList<>()).add(sale);
                    ...
                    if (sale.isFree()) freeLogs.add(new FreeItemLog(sale.getItemName(), sale.getSalesmanId(), sale.getSoldBy(), sale.getTimestamp()));
                }
            } else if (obj instanceof TreeMap) {
                salesByTime = (TreeMap<LocalDateTime, List<SaleTransaction>>) obj;
                // rebuild maps
                salesmanSales.clear();
                itemSales.clear();
                freeLogs.clear();
...
// ============================================================
// MENU PANEL (with Trie search & persistence fix)
// ============================================================
class MenuPanel extends JPanel {
    ...
    // Inner class: Trie for fast prefix search
    class MenuTrie {
        class TrieNode {
    ...
    public MenuPanel(DashboardPanel dashboard, TransactionLogPanel log,
        ...
        loadSavedMenuItems();          // Load saved data first
        if (menuItems.isEmpty()) {
            addDefaultItems();         // Only defaults if nothing saved
        }
        refreshMenuDisplay();
    }
    ...
    private void sellItem(MenuItem item) {
        ...
        // Push undo action to OrderPanel
        int oldTotal = dashboard.getTotalSales() - finalPrice;
        int oldHonesty = dashboard.getHonestyScore();
        Runnable undo = () -> {
            dashboard.addSale(-finalPrice);
            log.addEntry("Undo sale of " + item.getName());
        };
        mainFrame.orders.pushUndo(undo);
        ...
        if (menuFile.exists()) {
            try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(menuFile))) {
                ...
                if (obj instanceof Map) {
                    menuItems = new LinkedHashMap<>((Map<String, MenuItem>) obj);
                    // Rebuild trie
                    menuTrie = new MenuTrie();
                    for (MenuItem item : menuItems.values()) menuTrie.insert(item);
                }
...
// ============================================================
// SALESMAN PANEL (with max-heap top salesmen)
// ============================================================
class SalesmanPanel extends JPanel {
    ...
    private void closeShift() {
        ...
        // Use TreeMap range query for efficiency
